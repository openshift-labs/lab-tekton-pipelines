...
Similarly, start a pipeline in parallel in the second terminal to build and deploy the frontend application:

[source,bash,role=execute-2]
----
tkn pipeline start build-and-deploy -w name=shared-workspace,claimName=source-pvc -p deployment-name=pipelines-vote-ui -p git-url=https://github.com/openshift-pipelines/pipelines-vote-ui.git -p IMAGE=image-registry.openshift-image-registry.svc:5000/`oc project -q`/vote-ui --showlog
----
